---
layout: ../layouts/AboutLayout.astro
title: "About"
---

Hi, I'm Ayan. I work as a Developer Advocate, building agentic AI systems and writing about the part most people skip past: the live web-data layer that feeds them.

The hard part of a production AI agent isn't the model. It's the fresh, unblocked data you can get in front of it. Models keep getting cheaper and easier to swap, so the thing that decides whether an agent actually works is the harness around it, and that harness is only as good as the data it can reach. Most of my writing circles this one idea.

I came up as an embedded software engineer, electronics and instrumentation by training, and I still think like one: close to the hardware, slow to trust magic. That decade of writing code by hand is also what lets me notice when an agent quietly does the wrong thing, which happens more often than the demos let on.

This site is my archive: projects, experiments, and the tools I actually use, from AI and web data to the analog things I can't quit.

## Current Interests

Right now, I'm exploring:

* **Agentic workflows** — designing autonomous, tool-using systems that can think, act, and iterate with minimal intervention.
* **Web scraping systems** — building reliable pipelines to extract, clean, and structure data from the open web.
* **Self-hosting** — owning my stack, from infrastructure to applications, with a focus on privacy and control.

Alongside this, I consciously balance the digital with the analog—
vinyl records, mechanical watches, hi-fi audio systems, and the joy of driving manual cars.

## Featured In

Work featured on Hackaday, O'Reilly, and HackerNews. Published in Electronics For You (EFY), Open Source For You (OSFY), and the Mentor Graphics (Siemens) blog. Speaker at PyCon India 2017.

## ORCID iD

<a href="https://orcid.org/0009-0001-7765-7735" target="_blank" rel="me noopener noreferrer" style="display:inline-flex;align-items:center;gap:0.4em;">
  <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 256 256" aria-hidden="true" style="flex-shrink:0;">
    <path fill="#A6CE39" d="M256,128c0,70.7-57.3,128-128,128C57.3,256,0,198.7,0,128C0,57.3,57.3,0,128,0C198.7,0,256,57.3,256,128z"/>
    <path fill="#FFFFFF" d="M86.3,186.2H70.9V79.1h15.4v107.1z"/>
    <path fill="#FFFFFF" d="M108.9,79.1h41.6c39.6,0,57,28.3,57,53.6c0,27.5-21.5,53.6-56.8,53.6h-41.8V79.1z M124.3,172.4h24.5c34.9,0,42.9-26.5,42.9-39.7c0-21.5-13.7-39.7-43.7-39.7h-23.7V172.4z"/>
    <path fill="#FFFFFF" d="M88.7,56.8c0,5.5-4.5,10.1-10.1,10.1c-5.6,0-10.1-4.6-10.1-10.1c0-5.6,4.5-10.1,10.1-10.1C84.2,46.7,88.7,51.3,88.7,56.8z"/>
  </svg>
  <span>https://orcid.org/0009-0001-7765-7735</span>
</a>

## Background & technical skills

### Programming Languages

* Compiled: C/C++
* Scripting: Bash, Python, Node.js
* Markup: HTML
* Notation: JSON, XML

### Project Management & DevOps

* Issue Trackers: JIRA, BugZilla
* CI: Jenkins, Travis CI
* Code Management: GitHub, GitLab
* Containers: LXC, Docker

### Embedded Software & Design

* Custom OS: Yocto/OpenEmbedded, OpenWRT
* Automotive: AGL, GENIVI
* IDE: PlatformIO, Arduino, Eclipse, Atom, TI-CCS, Atmel Studio

### Hardware

* Microcontrollers: Intel 8051, Atmel AVR, ARM7 TDMI, ARM Cortex-M4, MSP430
* SBCs: Raspberry Pi, BeagleBone, Intel Galileo/Edison
* SoCs: Espressif ESP8266/ESP32, TI TIVA C, Cypress PSoC

### Protocols

* IoT: HTTP, MQTT, CoAP, LWM2M, WiFi, Bluetooth Classic & BLE
* Embedded: UART, MODBUS, I2C, SPI, 1-wire, CAN, LIN
* RF: Nordic NRF24, ZigBee/XBee, LoRa

## Hobbies

* Coffee brewing
* Vinyl records & hi-fi listening
* Mechanical watches
* Manual driving
* RC airplanes
* FPV racing drones / quadcopters
* Application-specific drones
